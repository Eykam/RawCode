...
public class Simulation {
	...
	public static Semaphore cookLock;
	public static List<Customer> orders;
	...
	public static List<SimulationEvent> runSimulation(
		...
		cookCount = numCooks;
		cookLock = new Semaphore(numCooks);
		orders = new ArrayList<Customer>();
		CountDownLatch orderLatch = new CountDownLatch(numCustomers);
		machines = new Machines[4];
		...
		for(int i = 0; i < machines.length; i++) {
			...
			switch (i) {
			case (0):
				currMachine = new Machines(Machines.MachineType.sodaMachines, FoodType.soda, machineCapacity);
				machines[i] = currMachine;
				break;
			case (1):
				currMachine = new Machines(Machines.MachineType.fryers, FoodType.fries, machineCapacity);
				machines[i] = currMachine;
				break;
			case (2):
				currMachine = new Machines(Machines.MachineType.grillPresses, FoodType.subs, machineCapacity);
				machines[i] = currMachine;
				break;
			case (3):
				currMachine = new Machines(Machines.MachineType.ovens, FoodType.pizza, machineCapacity);
				machines[i] = currMachine;
				break;
			}
			
			if(currMachine != null)
				logEvent(SimulationEvent.machinesStarting(currMachine, currMachine.machineFoodType, machineCapacity));
		...
		for(int i = 0; i < numCooks; i++) {
			cookThreads[i] = new Thread(new Cook(String.valueOf(i),orderLatch));
			cookThreads[i].start();
		}
		

		// Build the customers.
		Semaphore tableLock = new Semaphore(numTables);
		...
		if (!randomOrders) {
			for (int i = 0; i < customers.length; i++) {
				...
				customers[i] = new Thread(new Customer("Customer " + (i), order, tableLock));
			}
		} else {
			for (int i = 0; i < customers.length; i++) {
				...
				customers[i] = new Thread(
					new Customer("Customer " + (i), order, tableLock));
			}
		}
		...
		// Shut down machines
		for (int i=0; i < machines.length; i++) {
			Machines curr = machines[i];
			logEvent(SimulationEvent.machinesEnding(curr));
		}
	...
	public static void sendOrder(Customer curr) {
		synchronized(orders) {
			orders.add(curr);	
		}
	}
