(* $Id: util.ml,v 1.1 2022/09/08 18:56:59 kircher Exp $ *)
...
let execute_child timeout max_output child_program args input_string : child_status =
	...
	if ((!child_pid) = 0) then
		...
		let _ = handle_unix_error execv child_program args in ();
		...
		let child_output_buffer = Bytes.create (max_output + 1) in
		...
		
		{
			halted = (!halted) ;
			run_time = 0.0 ;   (* fixme *)
			child_output = Bytes.to_string (Bytes.sub child_output_buffer 0 (!output_string_position)); 
