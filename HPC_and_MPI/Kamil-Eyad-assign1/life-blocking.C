    ...
    for (int i = 1; i < X_limit + 1; i++) {
        for (int j = 1; j < Y_limit + 1; j++) {
            ...
            if(i == 1){
                ...
                // cout << "     Total Neighbors: " << neighbors << "\n";
            }else if(i == X_limit){
                // if(rank == 0){
                //     cout << "\nTop row: [" << i << "," << j << "]\n";
...
int main(int argc, char *argv[]) {
    ...
    for (int numg = 0; numg < num_of_generations; numg++) {
        ...
        if(rank != 0){
            MPI_Send(bottomSendRow, Y_limit, MPI_INT, rank-1, 0, MPI_COMM_WORLD); 
        }
        
        if(rank != size-1){
            MPI_Recv(topRecvRow, Y_limit, MPI_INT, rank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        ...
        if(rank != 0){
            MPI_Recv(bottomRecvRow, Y_limit, MPI_INT, rank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }

        compute(receivedLife, receivedPrev, bottomRecvRow, topRecvRow, rowsPerProcess, Y_limit, rank);
    }
    clock_t end = clock();
