...
module Sessions
	...
	def issue_session
		SecureRandom.random_number(4 ** SESSION_BITS)
	end
...
module Access
	...
	def authenticate(user, passwd)
		assign_session user if get_user(user)[:password] == passwd
	end
...
module User
	...
	def update_prefs(user, session, description, token)
		...
		if(authorize(user,session) && @tokens[user] == token)
...
module Posts
	...
	def get_posts(user)
		...
		#{"WHERE Posts.User = ?" if user}
		ORDER BY Posts.ID DESC
		}
		@db.execute(user == nil ? query : query,[user]) do |pos|
